...
- **Total vocabulary size**: 10,029 words
...
│  │         ▼                                                           │    │
│  │  ┌──────────────┐                                                  │    │
│  │  │ Linear Layer │  Input: 2048 → Output: 1024                      │    │
...
│  │         ▼                                                           │    │
│  │  ┌──────────────┐                                                  │    │
│  │  │  Embedding   │  vocab_size → embed_size (512)                   │    │
...
│  │  │  ┌────────────────────────────────────┐  │                      │    │
│  │  │  │  Input: embeddings (batch, seq, 512)│ │                      │    │
│  │  │  │  Hidden: h_t (2, batch, 1024)        │ │                      │    │
│  │  │  │  Cell: c_t (2, batch, 1024)          │ │                      │    │
│  │  │  │                                      │ │                      │    │
│  │  │  │  ┌─────────────────────────────┐    │ │                      │    │
│  │  │  │  │  Forget Gate (f_t)          │    │ │                      │    │
...
│  │         ▼                                                           │    │
│  │  ┌──────────────┐                                                  │    │
│  │  │ Linear Layer │  1024 → vocab_size (10029)                       │    │
...
**Output**: Hidden state vector (1024-dim) used to initialize LSTM
...
**Architecture**:
```python
Embedding(vocab_size, 512) → Dropout(0.5) → LSTM(512, 1024, num_layers=2) → Linear(1024, vocab_size)
```
...
The training shows:
1. **Rapid initial decrease** (Epochs 1-5): Loss drops from ~4.8 to ~3.5
2. **Steady improvement** (Epochs 5-25): Gradual decrease to ~2.9
3. **Convergence** (Epochs 25-35): Minimal improvement, model stabilizes around 2.5
...
| **1-gram Precision** | 0.4888 | Token-level precision |
| **1-gram Recall** | 0.4680 | Token-level recall |
| **1-gram F1** | 0.4649 | Harmonic mean |
| **ROUGE-L** | 0.4400 | Longest common subsequence |

**Key Observations**:
- Strong BLEU-1 score (0.60) indicates good word-level accuracy
- Decreasing BLEU scores (1→4) show difficulty with longer n-grams
- Balanced precision/recall suggests no significant bias
- Average caption length: ~11.7 words (close to ground truth: 12.3)
